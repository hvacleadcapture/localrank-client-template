...
// Build #19: extracted from src/app/lp/[slug]/page.tsx so the
// SITE_ROLE='landing_page' root page can reuse the renderer. Returns null
// when the LP can't be loaded; the caller decides whether to 404 (slug
// route) or fall back to a placeholder (root page).
export async function LandingPageView({
  ...
  const lp = await getLandingPage(slug, previewToken)
  if (!lp) return null
  ...
  const isPreview = !!previewToken && lp.status !== 'published'
